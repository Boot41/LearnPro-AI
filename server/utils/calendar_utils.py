...
CALENDAR_ENABLED = os.getenv("CALENDAR_ENABLED", "False").lower() in ["true", "1", "t"]

print(CALENDAR_ENABLED)

def create_calendar_event_recurring(event_details):
    if not CALENDAR_ENABLED:
        print("Calendar is disabled")
        return None
    else:
...
def create_calendar_event(event_details):
    if not CALENDAR_ENABLED:
        ...
        user_email = event_details.get("user_email")
        session_duration = event_details.get("session_duration")
